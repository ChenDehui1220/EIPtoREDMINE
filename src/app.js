...
$(function() {
    ...
    function collectHTML(form, url) {
        ...
        obj.eipNumber = $content.find('table:eq(0)').find('p:eq(1)').html().replace(regex, '');
        obj.completeDay = $content.find('#__希望完成日期-label').html();
        obj.desc = $content.find('table:eq(1)').find('tr:eq(4)').html().replace(regex, '');

        if (urlMapping[obj.moduleName] !== undefined) {
            chrome.storage.sync.set({ 'data': JSON.stringify(obj) });
            setTimeout(function() {
                window.open('http://pm.hq.hiiir/projects/'+urlMapping[obj.moduleName]+'/issues/new');
            }, 50);
        }
    }
